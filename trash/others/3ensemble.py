...
from utils import GridMask, AddNoise, imshow, imsave, ImbalancedDatasetSampler, calc_img_stats, image_result
...
imsave(img_grid, transform_mean, transform_std, 'target.png')
writer.add_image('Target Image', img_grid)
images = images.to(device)
# net
output = net(images)
img_grid = torchvision.utils.make_grid(output.cpu().data)
imsave(img_grid, transform_mean, transform_std, 'predict.png')
writer.add_image('Net Reconstruct Image', img_grid)
...
for epoch in range(args.ae_epoch):
    ...
    img_grid = torchvision.utils.make_grid(images)
    writer.add_image('Target Image', img_grid, epoch)
    images = images.to(device)
    # net
    output = net(images)
    img_grid = torchvision.utils.make_grid(output.cpu().data)
...
dataiter = iter(val_loader)
images, labels = dataiter.next()
img_grid = torchvision.utils.make_grid(images, normalize=True, range=(0, 1))
imsave(img_grid, transform_mean, transform_std, 'target.png')
writer.add_image('Target Image', img_grid, args.ae_epoch)
images = images.to(device)
image_result(images, net, net1, net2, transform_mean, transform_std, writer, args.ae_epoch)
