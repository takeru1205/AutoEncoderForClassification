...
# Log
writer = SummaryWriter(log_dir=f'sample/ensemble-{args.ae_epoch}-{args.train_epoch}')
...
train_set = ImbalancedCIFAR10(train_imbalance_class_ratio, transform=tmp_transform)

transform_mean, transform_std = calc_img_stats(DataLoader(train_set))
...
train1_dataset = Subset(train_set, train_indices)
train1_dataset.dataset.transform = train_transform
val_dataset = Subset(train_set, val_indices)
val_dataset.dataset.transform = evaluate_transform

train1_indices, train2_indices = separate_data(train1_dataset, train_indices)

train2_dataset = deepcopy(train1_dataset)
...
ae1_optimizer = optim.AdamW(itertools.chain(net1.encoder.parameters(), net1.decoder.parameters()), lr=0.0005, betas=(0.9, 0.999))
ae2_optimizer = optim.AdamW(itertools.chain(net2.encoder.parameters(), net2.decoder.parameters()), lr=0.0005, betas=(0.9, 0.999))
optimizer1 = optim.AdamW(itertools.chain(net1.encoder.parameters(), net1.classifier.parameters()), lr=0.0005, betas=(0.9, 0.999))
optimizer2 = optim.AdamW(itertools.chain(net2.encoder.parameters(), net2.classifier.parameters()), lr=0.0005, betas=(0.9, 0.999))
